...
import { BaseScheduler } from './scheduler-base';
import { maskState } from './mask';
function makeCell(id, overrides) {
    return {
        id,
        name: `Cell ${id}`,
...
function simpleCreateGlobals(_cellState, _env, _secrets, _secretsObj, _props, cellsApi, signal, onLog) {
    return {
...
class TestScheduler extends BaseScheduler {
    ...
    onResult;
    env;
    constructor(getCell, onResult, env) {
        super();
...
describe('BaseScheduler', () => {
    ...
    beforeEach(() => {
        ...
        results = [];
        scheduler = new TestScheduler((id) => (id === cell.id ? cell : undefined), (id, result) => results.push({ id, result }), () => ({ env: {}, secrets: new Set(), secretsObj: {} }));
    });
    afterEach(() => {
        vi.useRealTimers();
    });
    ...
    it('stopAll stops all running cells', async () => {
        ...
        scheduler = new TestScheduler((id) => (id === cell.id ? cell : id === cell2.id ? cell2 : undefined), (id, result) => results.push({ id, result }), () => ({ env: {}, secrets: new Set(), secretsObj: {} }));
        scheduler.start('cell-1');
...
//# sourceMappingURL=scheduler-base.test.js.map